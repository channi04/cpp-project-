...
int main(){
	...
	Mat oriDa = imread("dados.jpg", IMREAD_GRAYSCALE);
	Mat masc = imread("dados.jpg", IMREAD_GRAYSCALE);
	Mat chida = imread("cielo.jpg", IMREAD_GRAYSCALE);
	Mat image2;
	...
	VideoCapture cap(0);// abre la c�mara de default
	if (!cap.isOpened())//revisa si se abri� correctamente
		return -1;

	for(;;){
		Mat frame;

		cap >> frame;//obtiene un nuevo cuadro de la c�mara
